...
// ============================================================
// STORAGE LAYER (window.storage in artifact, fallback to memory)
// ============================================================
const storage = {
  async get(key) {
    try {
...
const initialState = {
  ...
  alumnos: [
    { id: 'a1', nombre: 'Luca Valentín', apellido: 'Perez', dni: '50123456', fechaNacimiento: '2012-04-15', celular: '1145678901', celularAlternativo: '1198765432', cursoId: 'c1', horarioCurso: 'Lunes y Miércoles · 18:00 a 19:30', grupoFamiliarId: 'gf1', activo: true, observaciones: '' },
    { id: 'a2', nombre: 'Sofía', apellido: 'Perez', dni: '52345678', fechaNacimiento: '2014-08-22', celular: '1145678901', celularAlternativo: '', cursoId: 'c2', horarioCurso: 'Martes y Jueves · 17:00 a 18:30', grupoFamiliarId: 'gf1', activo: true, observaciones: '' },
  ...
  gruposFamiliares: [{ id: 'gf1', nombre: 'Familia Pérez' }],
  promociones: [],
  pagos: [],
  configuracion: {
    nombreInstituto: 'IBO',
    ...
    plantillaWhatsApp: 'Hola {nombre}! Confirmamos el pago de {periodos} en {instituto} por un total de {total}. ¡Muchas gracias!',
    plantillaWhatsAppParcial: 'Hola {nombre}! Recibimos un pago parcial de {monto} para la cuota de {periodo} en {instituto}. Saldo pendiente: {saldo}. ¡Gracias!',
    plantillaWhatsAppSaldo: 'Hola {nombre}! Confirmamos el pago del saldo pendiente de {periodo} ({monto}) en {instituto}. ¡Cuota saldada!'
  }
};
...
const calcularCuota = (alumno, periodoId, anio, ctx) => {
  ...
  let etapa, recargoPct = 0, soloTransferencia = false;
  if (hoy <= dia15) {
  ...
  let efectivoFinal = soloTransferencia ? null : Math.round(efectivoBase * (1 + recargoPct / 100));
  let transferenciaFinal = Math.round(transferenciaBase * (1 + recargoPct / 100));

  // Descuento por hermano (escalonado según posición)
  ...
  if (alumno.grupoFamiliarId) {
    ...
    // Buscar el descuento más alto cuya posición sea <= a la del alumno
    const descuentos = (configuracion.descuentosHermanos || []).slice().sort((a, b) => a.posicion - b.posicion);
    descuentos.forEach(d => {
      if (posicion >= d.posicion) descuentoHermano = d.porcentaje;
    });

    if (descuentoHermano > 0) {
      if (efectivoFinal !== null) efectivoFinal = Math.round(efectivoFinal * (1 - descuentoHermano / 100));
...
export default function App() {
  ...
  useEffect(() => {
    if (loaded) storage.set('ibo_data', data);
  }, [data, loaded]);
    ...
    <>
      ...
      <div className="min-h-screen text-stone-900 font-body" style={{
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          <Header tab={tab} setTab={setTab} nombreInstituto={data.configuracion.nombreInstituto} />
          <div className="mt-6">
...
// HEADER
// ============================================================
function Header({ tab, setTab, nombreInstituto }) {
  const tabs = [
...
// ============================================================
function AlumnoPagoCard({ alumno, curso, data, anio, isSelected, togglePeriodo, onVerPago, onChange, onRemove, showRemove }) {
  return (
    <div className="bg-white rounded-2xl border border-stone-200 p-6">
      ...
      <div className="text-xs uppercase tracking-wider text-stone-500 mb-3">Período a pagar · {anio}</div>
        ...
        {PERIODOS.map(p => {
          ...
          return (
            <button
              ...
              title={
                estado === 'pagado' ? `${p.full} - PAGADO (click para ver/eliminar)` :
                estado === 'parcial' ? `${p.full} - PARCIAL · saldo ${fmtMoney(estadoCuota.saldo)} (click para ver)` :
                p.full
              }
              className={`py-3 px-2 rounded-lg border text-sm font-medium transition-all relative ${
                estado === 'pagado'
                  ? 'bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-500 hover:shadow-md hover:-translate-y-0.5 hover:ring-2 hover:ring-emerald-200 cursor-pointer'
                  : estado === 'parcial'
                    ? 'bg-amber-100 text-amber-800 border-amber-300 hover:bg-amber-200 hover:shadow-md hover:-translate-y-0.5 hover:ring-2 hover:ring-amber-200 cursor-pointer'
                    : seleccionado
                      ? 'bg-emerald-50 text-emerald-800 border-emerald-400 ring-2 ring-emerald-200'
                      : 'bg-white text-stone-700 border-stone-200 hover:border-emerald-300 hover:bg-emerald-50/30'
              }`}
            >
              {estado === 'pagado' && <Check size={12} className="inline mr-1 -mt-0.5" />}
              {p.label}
              {estado === 'parcial' && (
                <span className="absolute -top-1 -right-1 bg-amber-500 text-white text-[9px] font-bold px-1 rounded-full">½</span>
              )}
            </button>
          );
        })}
      ...
      <div className="mt-4 flex items-center gap-4 text-xs text-stone-500 flex-wrap">
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-emerald-600 inline-block"></span> Pagado</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-amber-100 border border-amber-300 inline-block"></span> Parcial</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-emerald-50 border border-emerald-400 inline-block"></span> Seleccionado</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-white border border-stone-200 inline-block"></span> Pendiente</span>
      </div>
    </div>
  );
}
...
// ============================================================
function PaymentModal({ data, selectedPeriodos, onClose, onConfirm }) {
  ...
  const generarMensajeIndividual = (alumno, items) => {
    const cfg = data.configuracion;
    const inst = cfg.nombreInstituto;
    // Si todos los items son pagos totales, usar plantilla normal
    const totales = items.filter(it => it.modalidad === 'total');
    ...
    if (totales.length > 0) {
      const periodos = totales.map(it => `${it.periodo.full} ${it.anio}`).join(', ');
      const total = totales.reduce((s, it) => s + it.monto, 0);
    ...
    parciales.forEach(it => {
      partes.push(replaceVars(cfg.plantillaWhatsAppParcial || 'Hola {nombre}! Recibimos un pago parcial de {monto} para la cuota de {periodo} en {instituto}. Saldo pendiente: {saldo}. ¡Gracias!', {
        nombre: alumno.nombre,
    ...
    saldos.forEach(it => {
      partes.push(replaceVars(cfg.plantillaWhatsAppSaldo || 'Hola {nombre}! Confirmamos el pago del saldo pendiente de {periodo} ({monto}) en {instituto}. ¡Cuota saldada!', {
        nombre: alumno.nombre,
        monto: fmtMoney(it.monto),
        periodo: `${it.periodo.full} ${it.anio}`,
        instituto: inst
  ...
  const generarMensajeAgrupado = (alumnos, items, total) => {
    // Para grupo familiar (mismo celular), construyo un mensaje conjunto
    const detalle = alumnos.map(a => {
      const itemsAlumno = items.filter(it => it.alumnoNombre === a.nombre);
      return itemsAlumno.map(it => {
        if (it.modalidad === 'parcial') return `• ${a.nombre}: pago parcial de ${it.periodo.full} (${fmtMoney(it.monto)}, saldo: ${fmtMoney(it.saldoRestante)})`;
        if (it.modalidad === 'saldo') return `• ${a.nombre}: saldo pendiente ${it.periodo.full} (${fmtMoney(it.monto)}) — saldada`;
  ...
  const handleConfirm = () => {
    ...
    const gruposArr = Object.values(grupos).map(g => ({
      ...
      mensaje: g.alumnos.length > 1
        ? generarMensajeAgrupado(g.alumnos, g.items, g.total)
        : generarMensajeIndividual(g.alumnos[0], g.items)
    }));
...
function AlumnosTab({ data, update }) {
  const [editing, setEditing] = useState(null); // alumno o 'new'
  const [query, setQuery] = useState('');
  const [filtroCurso, setFiltroCurso] = useState('todos'); // 'todos' | 'sin-curso' | cursoId
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-4">
        ...
        {/* Filtros: select curso + checkbox inactivos */}
                ...
                <div key={a.id} className={`flex items-center gap-3 px-5 py-3 hover:bg-stone-50 ${!a.activo ? 'opacity-50' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm flex items-center gap-2">
                      ...
                      {grupo && <span className="text-xs px-1.5 py-0.5 bg-emerald-50 text-emerald-700 rounded">{grupo.nombre}</span>}
                    </div>
      ...
      <div className="bg-white rounded-2xl max-w-md w-full my-8">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-2 pt-4 border-t border-stone-200">
            <button onClick={onClose} className="flex-1 px-4 py-2.5 rounded-lg border border-stone-200 hover:bg-stone-50 font-medium text-sm">Cancelar</button>
            <button
              onClick={() => onSave({ ...(curso || {}), nombre, horarios })}
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-5">
        ...
        <div>
          ...
          />
          <p className="text-xs text-stone-400 mt-1">
            Variables: <code className="bg-stone-100 px-1 rounded">{'{nombre}'}</code> <code className="bg-stone-100 px-1 rounded">{'{periodos}'}</code> <code className="bg-stone-100 px-1 rounded">{'{instituto}'}</code> <code className="bg-stone-100 px-1 rounded">{'{total}'}</code>
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-400 mt-1">Solo transferencia, sobre precio original</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-stone-500 mt-0.5">Definí qué porcentaje de descuento se aplica según la posición del hermano (ordenados por edad, mayor a menor).</p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="bg-stone-50 rounded-lg p-3 text-xs text-stone-600">
          <strong>Cómo funciona:</strong> los hermanos se ordenan por fecha de nacimiento (mayor a menor). El descuento que aplica para cada uno es el del registro con la posición más alta que sea ≤ a su número de hermano. Recordá hacer click en <strong>Guardar configuración</strong> al final.
        </div>
      </div>
